...
cpdef void standardizePi(float[:,::1] P, float[::1] f, float[:,::1] PiNorm, int t) \
		nogil:
	cdef:
		int m = P.shape[0]
		int n = P.shape[1]
...
cpdef void estimateDist(float[:,::1] C, float[:,::1] D) nogil:
	cdef:
		int n = C.shape[0]
		int i, j
	for i in range(n):
...
# Estimate Q-matrix
cpdef void estimateQ(float[:,::1] D, float[:,::1] Q, float[::1] Dsum) nogil:
	cdef:
		int n = D.shape[0]
		int i, j
	for i in range(n):
...
# New D-matrix
cpdef void updateD(float[:,::1] D0, float[:,::1] D, int pA, int pB) nogil:
	cdef:
		int n = D0.shape[0]
		int c = 0
		int i, j, d
