...
def readGzipBeagle(beagle, nUser, relFilter=None, post=False):
		...
		for line in f:
			if c < 0:
				...
				if relFilter is None:
					...
					likeMatrix[c, :] = list(compress(temp[3:], relFilter))
				if post:
					snpVector[c] = temp[0]
...
def readPlink(plink, epsilon, t):
	...
	indList = np.copy(snpClass.iid[:, 1]) # Save Sample IDs
	snpFile = snpClass.read(dtype=np.float32).val # Read PLINK files into memory
	m, n = snpFile.shape # Dimensions
	f = np.nanmean(snpFile, axis=0)/2 # Allele frequencies
	...
	likeMatrix = np.empty((3*m, n), dtype=np.float32)
	...
	# Multithreading
	threads = [threading.Thread(target=convertPlink, args=(likeMatrix, snpFile, chunk, chunk_N, epsilon)) for chunk in chunks]
	for thread in threads:
		thread.start()
	for thread in threads:
		thread.join()
